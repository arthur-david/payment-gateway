...
@Service
public class DepositAuthorizerService implements IAuthorizerService {

    private final AuthorizerApiClient authorizerApiClient;

    @Override
    public boolean isResponsible(AuthorizerPurpose authorizerPurpose) {
        return AuthorizerPurpose.DEPOSIT.equals(authorizerPurpose);
    }

    @Override
    public boolean authorize(@Valid GetAuthorizerDTO getAuthorizerDTO) {
        try {
            AuthorizerResponse response = authorizerApiClient.authorizeDeposit(
